...
namespace TapTap.Support {
    public class TapSupport {
        ...
        public static void Init(string serverUrl, string rootCategoryId, TapSupportDelegate supportDelegate) {
            if (string.IsNullOrEmpty(serverUrl)) {
                throw new ArgumentNullException(nameof(serverUrl));
            }
            if (string.IsNullOrEmpty(rootCategoryId)) {
                throw new ArgumentNullException(nameof(rootCategoryId));
            }
            platformWrapper?.Init(serverUrl, rootCategoryId, supportDelegate);
        }
