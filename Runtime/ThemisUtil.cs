...
namespace TapTap.TapDB
{
    public sealed class ThemisUtil
    {
        ...
        private static void _reportException(string name, string reason, string stackTrace, bool uncaught)
        {
            ...
            ReportException(name, reason, stackTrace, uncaught ? EXCEPTION_TYPE_UNCAUGHT : EXCEPTION_TYPE_CAUGHT, uncaught && _autoQuitApplicationAfterReport);
        }
      ...
      public static void ReportException(string name, string message, string stackTrace, int type, bool isQuitApp)
        {
            var command = new Command.Builder()
               .Service(TapDBConstants.TAPDB_SERVICE)
